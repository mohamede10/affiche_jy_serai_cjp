...
export default function GenerateurAttestationPage() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f2c] via-[#0e1a3a] to-[#1a1f4a] flex items-center justify-center p-6">
      <div className="max-w-2xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="bg-white p-6 text-center border-b">
          <Link href="/" className="text-gray-400 hover:text-gray-600 text-sm float-left">
            ← Accueil
          </Link>
          <h1 className="text-3xl font-black text-black">CJP #FODR 2026</h1>
          <p className="text-gray-500">Générateur d'attestation</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              {formData.statut === 'Partenaire' ? 'Logo du partenaire *' : 'Photo de profil *'}
            </label>
            <input
              type="file"
            ...
            {photoPreview && (
              <div className="mt-2 flex justify-center">
                <img 
                  src={photoPreview} 
                  alt={formData.statut === 'Partenaire' ? 'Logo' : 'Photo'} 
          ...
          <Link href="/" className="w-full bg-gray-600 text-white font-bold py-4 rounded-xl hover:bg-gray-700 transition text-center block">
            ← RETOUR À L'ACCUEIL
          </Link>
        </form>
      </div>
